...
class RecipesFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        selectedImageResource = arguments?.getInt("categoryImage") ?: 0
        ...
        filteredRecipes = filterRecipesByCategory(selectedCategory)

        adapter = RecipeAdapter(filteredRecipes, ::onRecipeClick)
        recipesRecyclerView.adapter = adapter
        recipesRecyclerView.layoutManager = LinearLayoutManager(requireContext())
    }

    private fun filterRecipesByCategory(category: String): List<Recipe> {
        return allRecipes.filter { it.category == category }
    }

    private fun onRecipeClick(recipe: Recipe) {
        val navController = findNavController()
        val bundle = Bundle().apply {
            putString("recipeName", recipe.dishName)
            putString("recipeInstructions", recipe.recipe)
            putStringArrayList("recipeIngredients", ArrayList(recipe.ingredients))
            putInt("recipeImage", recipe.imageResource)
            putInt("recipeCalories", recipe.nutritionalInfo.calories)
            putDouble("recipeProtein", recipe.nutritionalInfo.protein)
            putDouble("recipeFat", recipe.nutritionalInfo.fat)
            putDouble("recipeCarbs", recipe.nutritionalInfo.carbs)
        }
        navController.navigate(R.id.action_recipesFragment_to_completeRecipeFragment, bundle)
    }
}
